...
//        enemyReserve = new RealArmy();
    ...
    public void setFlankToSlot(boolean isAllWall, int index) {
        if(isAllWall) return;
        
        // fix lỗi
//        index += countByType(front, Army.Unit.Hoplite) + numberOfWall;
        index += countByType(front, Army.Unit.Swordsman);
        int slotForHop = (int) Math.ceil((reserve.getSword().size() * Swordsman.size) / (float) this.spacePerSlotForHI);
        ...
        int slotForSteam = (int) Math.ceil((reserve.getSpear().size() * Spearman.size) / (float) this.spacePerSlotForHI);
